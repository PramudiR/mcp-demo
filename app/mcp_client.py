...
class MCPClient:
    ...
    def load_servers(self, config_path: str) -> None:
        """Load server configuration from a JSON file (mcp_config.json)
        and creates an instance of each server
        (no active connection until 'start' though).
        ...
        self.servers = [
            MCPServer(name, config) for name, config in self.config[
                "mcpServers"
                ].items()
                ]

    async def start(self) -> List[PydanticTool]:
        """Starts each MCP server and returns the tools for
        each server formatted for Pydantic AI."""
        self.tools = []
    ...
    async def cleanup_servers(self) -> None:
        ...
        if cleanup_tasks:
            try:
                ...
                logging.warning("Warning during final cleanup: %s", e)
                raise
...
class MCPServer:
    ...
    async def initialize(self) -> None:
        ...
        if command is None:
            raise ValueError("The command must be a valid string command.")
        ...
        try:
            ...
            logging.error(f"Error initializing server {self.name}: %s", e)
            await self.cleanup()
            raise
    ...
    async def cleanup(self) -> None:
        ...
        async with self._cleanup_lock:
            try:
                ...
                logging.error("Error during cleanup of server, %s", self.name)
                logging.error(e)
                raise
